<template>
  <a :href="href" target="_blank" rel="noopener noreferrer" class="social-link" :class="cssClass" :aria-label="ariaLabel">
    <img v-if="isImageType" :src="imageSrc" :class="cssClass" :width="resolvedWidth" :height="resolvedHeight" :alt="ariaLabel" />
    <svg v-else v-html="svgContent" :class="cssClass" :width="resolvedWidth" :height="resolvedHeight" :viewBox="resolvedViewBox" aria-hidden="true" focusable="false"></svg>
  </a>
</template>
...
const SVG_DEFINITIONS: Record<string, SvgDef> = {
  linkedin: {
...
const def = computed(() => SVG_DEFINITIONS[props.type])
const isImageType = computed(() => !!def.value?.image)
const imageSrc = computed(() => def.value?.image ?? '')
const resolvedWidth = computed(() => def.value?.width ?? String(props.width ?? '50'))
const resolvedHeight = computed(() => def.value?.height ?? String(props.height ?? '50'))
const svgContent = computed(() => def.value?.content ?? '')
const resolvedViewBox = computed(() => props.viewBox ?? def.value?.viewBox ?? '0 0 50 50')
const ariaLabel = computed(() => ARIA_LABELS[props.type] ?? props.type)
</script>
