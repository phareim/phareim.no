...
**Strategy.** The script reads `~/github/almanac-design/tokens/tokens.json` and writes `assets/themes/_tokens-generated.css`. The generated file emits the canonical `--almanac-*` names that the *existing* `assets/themes/almanac.css` already aliases through to its `--theme-*` system (e.g. `--theme-text: var(--almanac-ink)`). So the generator's job is to overwrite the literal hex values currently hardcoded at the top of `almanac.css` (`--almanac-paper: #f4f0e8` etc.) with values fetched from the source of truth. Pages keep using `--theme-text` etc. unchanged.

The generated file is **committed** to phareim.no — Cloudflare CI does not have access to `~/github/almanac-design/`, so we ship the vendored file. The `predev`/`prebuild` script refreshes the vendored file locally; in CI, the script soft-fails and CI uses whatever was last committed. Drift is caught the next time the script runs locally (you'll see a non-empty `git diff assets/themes/_tokens-generated.css`).
...
- [ ] **Step 3: Make `almanac.css` import the generated tokens at the top, and delete the now-redundant hardcoded `--almanac-*` block**
...
Then find and DELETE the existing hardcoded `--almanac-paper: #f4f0e8; --almanac-ink: #1a1a1a; ...` block near the top of the file — it's now provided by `_tokens-generated.css`. Leave the `--theme-*` mappings intact: `--theme-text: var(--almanac-ink)`, `--theme-card-border: var(--almanac-rule-light)`, etc. — those are the public surface and pages already use them.

Sanity-check: open `_tokens-generated.css` and `almanac.css` side by side, confirm every `--almanac-*` var that almanac.css references is now defined in the generated file.
