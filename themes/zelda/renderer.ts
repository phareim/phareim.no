...
import { hash2, paintAmbient, paintTile } from './terrain'
import type { TerrainEnv, TerrainLight } from './terrain'
...
export function createRenderer(canvas: HTMLCanvasElement, world?: World): Renderer {
  ...
  const roomArea = new Map<string, string>()
  if (world) {
    for (const id of Object.keys(world.rooms)) {
      ...
      roomArea.set(room.id, room.area)
    }
  }
  ...
  function terrainEnv(state: GameState): TerrainEnv {
    return {
      tile,
      u: unit(),
      t: presentT,
      area: roomArea.get(state.room.id) ?? 'overworld',
      width: state.room.width,
      height: state.room.height,
      at: (x, y) => tileAt(state, x, y),
    }
  }

  function roomLights(state: GameState): TerrainLight[] {
    const lights: TerrainLight[] = []
    for (const p of roomPortals.get(state.room.id) ?? []) {
      lights.push({ x: p.x, y: p.y, color: 'gold' }, { x: p.x + p.w, y: p.y + p.h, color: 'gold' })
    }
    return lights
  }

  /** Paint the whole static room with its top-left at (ox, oy). */
  function paintStaticLayerAt(g: Ctx, state: GameState, ox: number, oy: number): void {
    const env = terrainEnv(state)
    for (let y = 0; y < state.room.height; y++) {
      for (let x = 0; x < state.room.width; x++) {
        paintTile(g, env, x, y, ox + x * tile, oy + y * tile)
      }
    }
    g.save()
    g.translate(ox, oy)
    paintAmbient(g, env, roomLights(state))
    g.restore()
  }

  function paintStaticLayer(g: Ctx, state: GameState): void {
    paintStaticLayerAt(g, state, 0, 0)
  }
