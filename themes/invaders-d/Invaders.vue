<template>
  <canvas ref="canvas" class="invaders-d-canvas"></canvas>
</template>
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < line.length; c++) {
      ...
      particles.push({
        ...
        vy: dy * sp + rand(-40, 40),
        life: 1,
        decay: 1.9 + Math.random() * 0.5, // ~0.5 s
        size: s,
        color,
      })
    }
...
function updateGame(dt, now) {
  ...
  if (ufo) {
    ...
    // Effect 7: light-streak samples behind the UFO (capped, dt-decayed).
    if (ufoTrail.length < 24) ufoTrail.push({ x: ufo.x, y: ufo.y, t: 0.4 })
    else {
      ufoTrail.shift()
...
function updateShot(dt, now) {
  if (!shot) return
  // Effect 5: short cyan trail (3-4 afterimages, decreasing alpha).
  shotTrail.push({ x: shot.x, y: shot.y, t: 0.18 })
  const maxTrail = mobileFx ? 3 : 4
  if (shotTrail.length > maxTrail) shotTrail.shift()
  shot.y -= shotSpeed() * dt
  if (shot.y < -20) {
  ...
  if (ufo) {
    ...
    if (shot.x > ufo.x - uw / 2 && shot.x < ufo.x + uw / 2 && shot.y > ufo.y && shot.y < ufo.y + uh) {
      ...
      for (let k = 0; k < 8; k++) {
        if (ufoTrail.length >= 24) ufoTrail.shift()
        ufoTrail.push({ x: ufo.x - ufo.dir * k * 9, y: ufo.y + uh / 2, t: 0.5 })
      }
...
function drawSprite(rows, ox, oy, s, color, o) {
  ...
  if (s === px) {
    ...
    // Kill flashes carry their own px after resize: direct glow, no shadow.
    const k = glowKey('rows:' + rows.join('|') + ':' + mainColor + ':' + s)
    let spr = glowCache.get(k)
    if (!spr) {
      const keepPx = px
      // Build at the flash's own pixel size without touching global px.
      const colsN = rows[0].length
      const w = colsN * s
      const h = rows.length * s
    ...
    blitSprite(spr, ox, oy)
  }
}

function drawBackground(now) {
  // Sky: deep violet-black with a magenta haze at the horizon.
  // (One linear gradient per frame; all radial gradients are pre-rendered.)
...
function drawBunkers() {
  ...
  const revealRow = (gh) => Math.floor(gh * (1 - revealP))
  for (let i = 0; i < bunkers.length; i++) {
    const b = bunkers[i]
    const showFrom = bunkerRevealT > 0 ? revealRow(b.gh) : 0
    // Find jolts for this bunker (Effect 6: 1 px electric jitter).
    let jx = 0
    let jy = 0
    let jflick = 1
    ...
    if (bunkerCellSprite && b.cell === bunkerCellPx) {
      const pad = bunkerCellSprite.pad
      for (let gy = showFrom; gy < b.gh; gy++) {
        const isfrontier = bunkerRevealT > 0 && gy === showFrom
        for (let gx = 0; gx < b.gw; gx++) {
          if (!b.grid[gy * b.gw + gx]) continue
          // Only cells near a hit jitter (within ~6 cells).
          let ox = 0
          let oy = 0
          let a = 1
          if (jx !== 0 || jy !== 0) {
            ...
            for (let j = 0; j < bunkerJolts.length; j++) {
              const jl = bunkerJolts[j]
              if (jl.bi !== i) continue
              const jgx = (jl.sx - b.x) / b.cell
              const jgy = (jl.sy - b.y) / b.cell
              if (Math.abs(gx - jgx) < 6 && Math.abs(gy - jgy) < 6) { near = true; break }
            }
            if (near) { ox = jx; oy = jy; a = jflick }
          }
          if (a !== 1) ctx.globalAlpha *= a
          ctx.drawImage(
            bunkerCellSprite.c,
            Math.round(b.x + gx * b.cell - pad + ox),
            Math.round(b.y + gy * b.cell - pad + oy)
          )
          if (a !== 1) ctx.globalAlpha /= a
          if (isfrontier) {
      ...
      // Fallback (cell size changed mid-frame): plain fills, no shadow.
      ctx.fillStyle = CYAN
      for (let gy = showFrom; gy < b.gh; gy++) {
        for (let gx = 0; gx < b.gw; gx++) {
          if (b.grid[gy * b.gw + gx]) {
...
function draw() {
  ...
  for (let i = 0; i < ufoPopups.length; i++) {
    ...
    // Scale pop: ~1.35x at birth, easing to 1x over ~0.25 s.
    const pop = 1 + 0.35 * Math.max(0, 1 - p.t / 0.25)
    ctx.font = `bold ${Math.round(Math.max(12, px * 3) * pop)}px "Courier New", monospace`
