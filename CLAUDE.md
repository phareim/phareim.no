...
platformer inspired by Another World's flat polygon landscapes. Added first
in the live theme rotation (ten themes, seven live). `themes/anotherworld/`
contains the design brief and shared types, a pure mutable physics engine,
Canvas 2D renderer, and Vue landing/control shell. The existing themes keep
their order relative to each other.

The authored coast has gaps, hazards, checkpoint signals and a finish.
Idle mode runs the same simulation on autopilot; reduced motion leaves the
idle scene still. Explicit play takes keyboard/touch control. The game stays
horizontal on phones, with a cropped following camera. No external assets,
network calls or new dependencies. Palette tokens also cover content routes
and a dedicated 404 block. See `themes/anotherworld/DESIGN.md` for the visual
and module contracts.

`npm run test:anotherworld` runs the physics regression suite, also in CI.
Muse implemented physics/tests, rendering and the landing shell in separate
worktrees; Codex provided the design, integration and review.

Verified 2026-09-06: all ten physics tests pass, including a complete demo
traversal. Chromium checks cover 1440×900, 375×667 and 667×375 layouts,
keyboard navigation lock, pause/resume, blur pause, exit, mobile multi-touch
and cancellation, reduced-motion idle, and the four shared/error routes.
Move with arrows or A/D, jump with Space/Up/W (hold for a longer jump),
pause with P/Escape. The vertical camera stays fixed to keep landing edges
above touch controls. The mobile idle moon sits below the profile copy.
